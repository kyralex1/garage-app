...
export const PRESETS_BY_TYPE = (t) =>
...
export const fmtKm = (n) => (n == null ? '—' : n.toLocaleString('de-DE') + ' km');
...
export function deriveDue(task, vehicle, now = new Date()) {
...
export function StoreProvider({ children }) {
  ...
  React.useEffect(() => {
    ...
    const onLocal = () => setData(loadStore());
  ...
  const api = React.useMemo(() => ({
    ...
    upcoming: (limit = 99) => {
      ...
      items.sort((a, b) => (a.due.effDays ?? 99999) - (b.due.effDays ?? 99999));
...
export async function processUploadedImage(file) {
  ...
  try {
    ...
    const isLineArt = whiteCount / (d.length / 4) > 0.3;
