...
import { useStore, deriveDue, fmtDate, fmtKm, fmtEur, todayISO, PRESETS, PRESETS_BY_TYPE, processUploadedImage } from '../store';
import { Icon, KIND_ICON, VehicleArt } from '../art';
import { C, FONT, MONO, StatusPill, HeaderBar } from './ScreensMain';

export function ScreenDetail({ vehicleId, go, accent }) {
      ...
      {open && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => {
              store.completeTask(task.id, vehicle.mileage);
            }} style={{
...
export function ScreenAdd({ vehicleId, go }) {
  const store = useStore();
  const vehicle = store.getVehicle(vehicleId);
        ...
        <Section label="Bezeichnung">
          <Field>
            <input value={title} onChange={e => setTitle(e.target.value)}
              placeholder="z. B. Ölwechsel" style={fieldStyle}/>
          </Field>
        </Section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <Field label="ALLE … KM">
              <input value={intervalKm} onChange={e => setIntervalKm(e.target.value.replace(/\D/g, ''))}
                placeholder="—" inputMode="numeric" style={fieldStyle}/>
            </Field>
        ...
        <Section label="Kosten · optional">
          <Field label="EUR">
            <input value={cost} onChange={e => setCost(e.target.value.replace(/[^\d.,]/g, '').replace(',', '.'))}
              placeholder="0.00" inputMode="decimal" style={fieldStyle}/>
          </Field>
        </Section>
...
export function ScreenNotifications({ go }) {
  const store = useStore();
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 4px 8px' }}>
        <span style={{ width: 6, height: 6, borderRadius: '50%', background: tone, boxShadow: `0 0 8px ${tone}`, display: 'inline-block' }}/>
...
export function ScreenAddVehicle({ type, go }) {
  const store = useStore();
  ...
  const [customImg, setCustomImg] = React.useState(null);
  ...
  const onPickFile = async (e) => {
    ...
    setUploading(true);
    try {
      const res = await processUploadedImage(f);
      setCustomImg(res);
    } catch (err) {
  ...
  const canSave = name.trim().length > 0 && store.countByType(type) < 10;
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column' }}>
      <HeaderBar
        ...
        sub={`${store.countByType(type)} / 10 belegt`}
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '12px 14px 24px' }}>
        ...
        <Section label="Bild">
          ...
          <div style={{ fontFamily: MONO, fontSize: 9.5, color: C.muted, marginTop: 6, lineHeight: 1.4 }}>
            Optional. Linework-Skizzen werden automatisch freigestellt; Fotos werden direkt verwendet.
          </div>
        </Section>
            ...
            {presets.map(p => (
              <button key={p.key} onClick={() => setPresetKey(p.key)} style={{
                ...
                <div style={{
                  ...
                  color: presetKey === p.key ? 'var(--accent)' : C.text,
                }}>{p.label}</div>
              </button>
            ))}
          </div>
        ...
        }}>{store.countByType(type) >= 10 ? 'Stellplatz voll (max. 10)' : 'Fahrzeug hinzufügen'}</button>
      </div>
    </div>
  );
}
