// Garage app — screens.
// All screens are pure functions of (route, store). Routing is via useRoute() in app.jsx.
...
// Header bar (shared)
function HeaderBar({ title, sub, onBack, action, eyebrow }) {
  return (
    <div style={{
...
// ─────────────────────────────────────────────────────────────
// SCREEN: Garage closed (door + open button)
// ─────────────────────────────────────────────────────────────
function ScreenClosed({ onOpen, accent, doorSpeed, doorOpen, onDoorEnd }) {
  return (
    <div style={{ position: 'relative', height: '100%', background: C.bg, overflow: 'hidden' }}>
      {/* Garage interior background — visible behind door once open */}
...
// Garage interior — shown behind door
function GarageInterior({ accent }) {
  return (
    <div style={{
      position: 'absolute', inset: 0,
      background: `
        radial-gradient(ellipse at 50% 30%, #1a1c22 0%, #0a0b0d 60%),
        #0a0b0d
      `,
    }}>
      {/* perspective floor */}
...
// ─────────────────────────────────────────────────────────────
// SCREEN: Garage open (vehicles + termin-ticker)
// ─────────────────────────────────────────────────────────────
function ScreenGarage({ go, accent }) {
...
// Garage zone — Car / Bike row. Shows up to N art tiles inline + count.
...
function ZoneArtStack({ list, type, accent }) {
  // Show up to 2 art tiles side-by-side (no overlap), "+N" if more.
  // Drops the first item so a 3-bike garage shows the middle + last;
  // a single bike just shows itself.
...
// ─────────────────────────────────────────────────────────────
// SCREEN: Vehicle list (cars or bikes)
// ─────────────────────────────────────────────────────────────
function ScreenList({ type, go, accent }) {
  const store = useStore();
  ...
  const canAdd = list.length < MAX_PER_TYPE;
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column' }}>
      <HeaderBar
        eyebrow={eyebrow}
        title={title}
        sub={`${list.length} / ${MAX_PER_TYPE} Fahrzeuge`}
        onBack={() => go('garage')}
        action={
...
function Stat({ label, value, accent }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 8.5, letterSpacing: 1.5, color: C.dim }}>{label}</div>
      <div style={{ fontSize: 12.5, color: accent || C.text, marginTop: 1, fontWeight: 500 }}>{value}</div>
    </div>
  );
}

Object.assign(window, {
  ScreenClosed, ScreenGarage, ScreenList,
  HeaderBar, StatusPill, Stat, VehicleCard, UpcomingRow,
  C, FONT, MONO,
});
